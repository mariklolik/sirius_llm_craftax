...
if __name__ == "__main__":
    ...
    while not game_finished:
        ...
        final_task = curriculum_agent.propose_next_task(state, exploration_progress)
        

        subtasks = curriculum_agent.subtasks(state, final_task)
        for task in subtasks:
            ...
            for i in range(4):
                ...
                code = action_agent.generate_code(task,code,execution_errors,critique,skills)
                execution_errors = invoke_action(state, code)
                success, critique = critic_agent.check_task_success(task, state)
                if success :
                    break
            if success :
