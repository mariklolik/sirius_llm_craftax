...
def mine_stone(env):
    ...
    if env.saved_state.inventory.stone < 20:
        # Если нет, ищем ближайший источник камня
        explore_until(
            env,
            lambda x: env.saved_state.inventory.stone >= 20,
            max_iter=25
        )
    ...
    while True:
        ...
        move_to_pos(
            env, target_pos=find_nearest_stone_source(env)
        )

        # Добываем камень
        mine_block(
            env, block_type=BlockType.STONE, count=1, can_dig=True, can_place=False
        )
