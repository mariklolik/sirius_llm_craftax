...
class CurriculumAgent:
    def __init__(self, logs_run):
        ...
        with open(
            "user_promts/curriculum_qa_step1.txt", encoding="utf-8"
        ) as file:
        ...
        with open(
            "user_promts/curriculum_qa_step2.txt", encoding="utf-8"
        ) as file:
        ...
        with open(
            "system_promts/curriculum_task_decomposition.txt", encoding="utf-8"
        ) as file:
            self.task_decomp_system_promt = file.read()
        with open(
            "user_promts/curriculum_task_decomposition.txt", encoding="utf-8"
        ) as file:
            self.task_decomp_user_promt = file.read()

    def get_exploration_progress(self, state):

        user_promt = format_text_with_state(
            self.qa_step_1_promt,
            state,
            self.completed_tasks,
            self.failed_tasks,
        )
                ...
                [
                    {
                        "role": "system",
                        "text": self.tutorial
                        + self.qa_step_2_promt_system_promt,
                    },
                    {
                        "role": "user",
                        "text": user_promt,
    ...
    def task_decomposition(self, state, task):
        task_decomp_user_promt = format_text_with_state(
            self.task_decomp_user_promt, state, task
        )
